var pJS = function(e, t) {
    ...
    t && Object.deepExtend(p, t), p.tmp.obj = {
        ...
        this.radius = (p.particles.size.random ? Math.random() : 1) * p.particles.size.value, p.particles.size.anim.enable && (this.size_status = !1, this.vs = p.particles.size.anim.speed / 100, p.particles.size.anim.sync || (this.vs = this.vs * Math.random())), this.x = a ? a.x : Math.random() * p.canvas.w, this.y = a ? a.y : Math.random() * p.canvas.h, this.x > p.canvas.w - 2 * this.radius ? this.x = this.x - this.radius : this.x < 2 * this.radius && (this.x = this.x + this.radius), this.y > p.canvas.h - 2 * this.radius ? this.y = this.y - this.radius : this.y < 2 * this.radius && (this.y = this.y + this.radius), p.particles.move.bounce && p.fn.vendors.checkOverlap(this, a), this.color = {}, "object" == typeof e.value ? e.value instanceof Array ? (a = e.value[Math.floor(Math.random() * p.particles.color.value.length)], this.color.rgb = hexToRgb(a)) : (null != e.value.r && null != e.value.g && null != e.value.b && (this.color.rgb = {
            r: e.value.r,
            g: e.value.g,
            b: e.value.b
        ...
        } : "string" == typeof e.value && (this.color = e, this.color.rgb = hexToRgb(this.color.value)), this.opacity = (p.particles.opacity.random ? Math.random() : 1) * p.particles.opacity.value, p.particles.opacity.anim.enable && (this.opacity_status = !1, this.vo = p.particles.opacity.anim.speed / 100, p.particles.opacity.anim.sync || (this.vo = this.vo * Math.random()));
        var i = {};
        ...
        p.particles.move.straight ? (this.vx = i.x, this.vy = i.y, p.particles.move.random && (this.vx = this.vx * Math.random(), this.vy = this.vy * Math.random())) : (this.vx = i.x + Math.random() - .5, this.vy = i.y + Math.random() - .5), this.vx_i = this.vx, this.vy_i = this.vy;
        a = p.particles.shape.type;
        "object" == typeof a ? a instanceof Array && (e = a[Math.floor(Math.random() * a.length)], this.shape = e) : this.shape = a, "image" == this.shape && (e = p.particles.shape, this.img = {
            src: e.image.src,
        ...
        var e, t, a, i = this;
        switch (e = null != i.radius_bubble ? i.radius_bubble : i.radius, t = null != i.opacity_bubble ? i.opacity_bubble : i.opacity, t = i.color.rgb ? "rgba(" + i.color.rgb.r + "," + i.color.rgb.g + "," + i.color.rgb.b + "," + t + ")" : "hsla(" + i.color.hsl.h + "," + i.color.hsl.s + "%," + i.color.hsl.l + "%," + t + ")", p.canvas.ctx.fillStyle = t, p.canvas.ctx.beginPath(), i.shape) {
            case "circle":
            ...
            case "triangle":
                ...
                break;
            case "polygon":
                p.fn.vendors.drawShape(p.canvas.ctx, i.x - e / (p.particles.shape.polygon.nb_sides / 3.5), i.y - e / .76, 2.66 * e / (p.particles.shape.polygon.nb_sides / 3), p.particles.shape.polygon.nb_sides, 1);
                break;
            case "star":
        ...
        for (var e = 0; e < p.particles.array.length; e++) {
            var t, a = p.particles.array[e];
            if (p.particles.move.enable && (t = p.particles.move.speed / 2, a.x += a.vx * t, a.y += a.vy * t), p.particles.opacity.anim.enable && (1 == a.opacity_status ? (a.opacity >= p.particles.opacity.value && (a.opacity_status = !1), a.opacity += a.vo) : (a.opacity <= p.particles.opacity.anim.opacity_min && (a.opacity_status = !0), a.opacity -= a.vo), a.opacity < 0) && (a.opacity = 0), p.particles.size.anim.enable && (1 == a.size_status ? (a.radius >= p.particles.size.value && (a.size_status = !1), a.radius += a.vs) : (a.radius <= p.particles.size.anim.size_min && (a.size_status = !0), a.radius -= a.vs), a.radius < 0) && (a.radius = 0), t = "bounce" == p.particles.move.out_mode ? {
                    x_left: a.radius,
                    x_right: p.canvas.w,
                    y_top: a.radius,
                    y_bottom: p.canvas.h
        ...
        var a = e.x - t.x,
            i = e.y - t.y,
            a = Math.sqrt(a * a + i * i);
        a <= p.particles.line_linked.distance && 0 < (i = p.particles.line_linked.opacity - a / (1 / p.particles.line_linked.opacity) / p.particles.line_linked.distance) && (a = p.particles.line_linked.color_rgb_line, p.canvas.ctx.strokeStyle = "rgba(" + a.r + "," + a.g + "," + a.b + "," + i + ")", p.canvas.ctx.lineWidth = p.particles.line_linked.width, p.canvas.ctx.beginPath(), p.canvas.ctx.moveTo(e.x, e.y), p.canvas.ctx.lineTo(t.x, t.y), p.canvas.ctx.stroke(), p.canvas.ctx.closePath())
        ...
        p.tmp.pushing = !0;
        for (var a = 0; a < e; a++) p.particles.array.push(new p.fn.particle(p.particles.color, p.particles.opacity.value, {
        ...
        "window" == p.interactivity.detect_on ? p.interactivity.el = window : p.interactivity.el = p.canvas.el, (p.interactivity.events.onhover.enable || p.interactivity.events.onclick.enable) && (p.interactivity.el.addEventListener("mousemove", function(e) {
            var t;
            e = p.interactivity.el == window ? (t = e.clientX, e.clientY) : (t = e.offsetX || e.clientX, e.offsetY || e.clientY), p.interactivity.mouse.pos_x = t, p.interactivity.mouse.pos_y = e, p.tmp.retina && (p.interactivity.mouse.pos_x *= p.canvas.pxratio, p.interactivity.mouse.pos_y *= p.canvas.pxratio), p.interactivity.status = "mousemove"
        ...
        var e;
        p.particles.number.density.enable && (e = p.canvas.el.width * p.canvas.el.height / 1e3, p.tmp.retina && (e /= 2 * p.canvas.pxratio), e = e * p.particles.number.value / p.particles.number.density.value_area, (e = p.particles.array.length - e) < 0 ? p.fn.modes.pushParticles(Math.abs(e)) : p.fn.modes.removeParticles(e))
        ...
        var t, a;
        p.tmp.img_error = void 0, "" != p.particles.shape.image.src ? "svg" == e ? ((t = new XMLHttpRequest).open("GET", p.particles.shape.image.src), t.onreadystatechange = function(e) {
        ...
        }), a.src = p.particles.shape.image.src) : (console.log("Error pJS - No image.src"), p.tmp.img_error = !0)
